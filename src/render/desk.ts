 ...
 */
/**
 * 책상 앞 모서리의 높이 — 이 아래는 통짜 갈색이다.
 *
 * ★ 화면 비율로 잡는다. 예전엔 30픽셀 고정이었다.
 *
 *   폰은 화면이 231픽셀밖에 안 돼서 30이면 13%를 먹는다. 데스크톱은
 *   480이라 같은 30이 6%다. 같은 숫자가 기기에 따라 두 배 넘게 다르게
 *   느껴진다 — 화면을 나눠 쓰는 값은 픽셀이 아니라 비율이어야 한다.
 *
 *   카메라가 책상선을 화면 86% 지점에 붙들어 두니(DESK_LINE), 나머지
 *   14% 안에서 상판과 앞면이 나눠 가진다. 앞면이 그 절반쯤이면
 *   상판이 남아서 책상처럼 보인다.
 */
export const LEDGE_Y = () => VIEW_H - Math.max(16, Math.round(VIEW_H * 0.065));
